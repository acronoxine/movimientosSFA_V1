...
if($quincena < 25)
{
	...
	$res = mysql_query($sql, $conexion);
	/*$delete=mysql_query("DELETE FROM nominapago WHERE cve_concepto='274'");
...
$sql = "select ur,id, nombre, sueldobase, compensacion, prima, aguinaldo, otrasper, isr, imss, otrasded,"; 
$sql .= " (sueldobase + compensacion + prima + aguinaldo + otrasper) - (isr + imss + otrasded) as neto from";
$sql .= " (select ur, id, nombre, "; 
...
$sql .= " from";
$sql .= " (select e.idnominaemp as id,a.idarea as ur, concat(e.paterno, ' ', e.materno, ' ', e.nombres) as nombre, p.cve_concepto, c.descripcion as concepto, p.tipo, p.importe"; 
...
include("./fpdf/fpdf.php");
	...
	/*	
		...
		$pdf->cell(17,10,number_format($importe_imss, 2, ".", ","), 1, 0, 'R');
		$pdf->cell(22,10,number_format($ren["otrasded"], 2, ".", ","), 1, 0, 'R');
		$pdf->cell(22,10,number_format($ren["neto"]-$importe_imss, 2, ".", ","), 1, 0, 'R');
