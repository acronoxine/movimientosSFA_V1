...
if(isset($_POST["aguinaldo"]) && $_POST["aguinaldo"] == "C")
{
	...
	$sql.= "SELECT n.idnominaemp,'115' AS cve_concepto,
	 (((((SUM(m.importe)*2)/30)*40)/365)*(DATEDIFF('2014-12-31',n.fechainicio)+1)) AS importe,";
	$sql.=" 'P' as tipo,'$quincena' AS quincena, '" . date("Y") . "' as anio,";
	$sql.=" CURDATE() AS fechaemision, CURDATE() AS fechasistema,"; 
	$sql.=" '$_SESSION[m_idregistro]' AS usuario, '002' AS cve_sat, n.area";
	$sql.=" FROM nominaemp n INNER JOIN nomina m ON n.idnominaemp=m.idnominaemp";
	$sql.=" WHERE (n.activo='1' AND m.concepto=101) OR (n.activo='1' AND m.concepto=114) 
GROUP BY n.idnominaemp";
	//echo $sql."<br>----------------<br>";
	$res = mysql_query($sql, $conexion);
...
if((isset($_POST["aguinaldo"]) && $_POST["aguinaldo"] == "1P") || $quincena == 25)
{
	...
	$sql .= " where n.activo = '1'";*/
	$sql.= "SELECT n.idnominaemp,'115' AS cve_concepto,
	 ((((((SUM(m.importe)*2)/30)*40)/365)*(DATEDIFF('2014-12-31',n.fechainicio)+1))/2) AS importe,";
	$sql.=" 'P' as tipo,'$quincena' AS quincena, '" . date("Y") . "' as anio,";
	$sql.=" CURDATE() AS fechaemision, CURDATE() AS fechasistema,"; 
	$sql.=" '$_SESSION[m_idregistro]' AS usuario, '002' AS cve_sat, n.area";
	$sql.=" FROM nominaemp n INNER JOIN nomina m ON n.idnominaemp=m.idnominaemp";
...
$sql .= " where quincena = '$quincena' and anio = '" . date("Y") . "' $ur and e.activo = '1') x group by ur, nombre) y order by ur, nombre";
...
include("./fpdf/fpdf.php");
	...
	while($ren = mysql_fetch_array($res))
	{
		...
		if($otraur != $ren["ur"])
		{
			...
			include("membrete_prenomina.php");
