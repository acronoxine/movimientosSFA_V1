...
$sql = "select q.quincena, q.fdesde, n.fechabaja from nominaemp n";
$sql .= " left join cat_quincenas q on n.fechabaja between q.fdesde and q.fhasta";
$sql .= " where idnominaemp = '$_GET[idnominaemp]'";

$res = mysql_query($sql, $conexion);
$ren = mysql_fetch_array($res);
$quincena = $ren["quincena"];

$fdesde = $ren["fdesde"];
$fechabaja = $ren["fechabaja"];
...
$res = @mysql_query($sql, $conexion);
//echo $sql."<br><br>---- crea tabla---<br>";

// CONCEPTOS DE LA NOMINA ORDINARIA
...
$sql .= " select n.idnominaemp, n.concepto, case when n.tipo = 'P' then (n.importe/15) * datediff(e.fechabaja, q.fdesde) else n.importe end as importe, n.tipo, '$quincena', '" . date("Y") . "', curdate(), '1', c.cve_sat, e.area";
$sql .= " from nomina n"; 
$sql .= " left join cat_conceptos c on n.concepto = c.clave";
$sql .= " left join nominaemp e on n.idnominaemp = e.idnominaemp and e.activo = '0'";
$sql .= " left join cat_quincenas q on e.fechabaja between q.fdesde and q.fhasta";
$sql .= " where n.idnominaemp = '$_GET[idnominaemp]'";
$res = mysql_query($sql, $conexion);

//echo $sql."<br><br>---- nomina ordinaria---<br>";
// AGUINALDO
...
$sql .= " else";
$sql .= "     0";
$sql .= " end / 365) * dayofyear(n.fechabaja) as importe,";
$sql .= " 'P' as tipo,";
...
$sql .= " where n.activo = '0' and n.idnominaemp = '$_GET[idnominaemp]'";
$res = mysql_query($sql, $conexion);
//echo $sql."<br><br>---- aguinaldo---<br>";
// ISR DEL AGUINALDO
...
$sql .= " c.cve_sat, n.area";
$sql .= " from nominaemp n";
$sql .= " left join cat_conceptos c on c.clave = '115' where n.activo = '0' and n.idnominaemp = '$_GET[idnominaemp]') x) y";
$sql .= " left join isranual i on ((y.importe - y.salmin) between i.limiteinferior and i.limitesuperior) or ((y.importe - y.salmin) >= i.limiteinferior and i.limitesuperior = 0)) z";
$res = mysql_query($sql, $conexion);

//echo $sql."<br><br>---- isr  aguinaldo---<br>";
// PRIMA VACACIONAL
...
$sql .= " where n.activo = '0' and n.idnominaemp = '$_GET[idnominaemp]'";
$res = mysql_query($sql, $conexion);
//echo $sql."<br><br>----prima vacacional ---<br>";
...
$sql .= " c.cve_sat, n.area";
$sql .= " from nominaemp n";
$sql .= " left join cat_conceptos c on c.clave = '119' where n.activo = '0' and n.idnominaemp = '$_GET[idnominaemp]') x) y";
$sql .= " left join isranual i on ((y.importe - y.salmin) between i.limiteinferior and i.limitesuperior) or ((y.importe - y.salmin) >= i.limiteinferior and i.limitesuperior = 0)) z";
$res = mysql_query($sql, $conexion);
//echo $sql."<br>";
...
$sql .= " where p.quincena = '$quincena' and p.anio = '" . date("Y") . "' and e.activo = '0' and e.idnominaemp = '$_GET[idnominaemp]'";
...
include("/fpdf/fpdf.php");
	...
	while($ren_emp = mysql_fetch_array($res_emp))
	{
		...
		$sql .= " where n.quincena = '$quincena' and n.anio = '" . date("Y") . "' and n.idnominaemp = '$ren_emp[idnominaemp]' and c.clave!=101 and c.clave!=114 and c.clave!=251 and c.clave!=274";
	...
	/********************************** copia ***********************************************************************/	
		...
		$sql = "select n.tipo, n.cve_concepto, c.descripcion as concepto, sum(n.importe) as importe  from nominapago_temp n";
		$sql .= " left join cat_conceptos c on n.cve_concepto = c.clave";
		$sql .= " where n.quincena = '$quincena' and n.anio = '" . date("Y") . "' and n.idnominaemp = '$ren_emp[idnominaemp]'";
		$sql .= " group by n.tipo, n.cve_concepto, c.descripcion";
		$res = mysql_query($sql, $conexion);
...
echo "<script>";
echo "window.location.replace('pdfs/doc.pdf');";
echo "</script>";
?>
