...
$query_empleado = "SELECT idnominaemp, case when n.activo = '1' then 'SI' else 'NO' end as activo, concat(ifnull(rfc_iniciales, ''), ifnull(rfc_fechanac, ''), ifnull(rfc_homoclave, '')) as rfc, curp, folio, paterno, materno, nombres, calle, numint, numext, colonia, cp, mun.municipio as ciudad, est.estado";
$query_empleado .= " , curp, a.descripcion as area, c.descripcion as categoria, n.sueldobase, plaza, concat(lpad(day(fechanacimiento), 2, '0'), '/', lpad(month(fechanacimiento), 2, '0'), '/', year(fechanacimiento)) as fechanacimiento, case when sexo = 'M' then 'MASCULINO' when sexo = 'F' then 'FEMENINO' else '' end as sexo,";
...
$query_empleado .= " left join cat_categoria c on n.categoria = c.idcategoria"; 
$query_empleado .= " left join cat_programa p on n.programa = p.idprograma"; 
$query_empleado .= " left join cat_area a on n.area = a.idarea"; 
$query_empleado .= " left join cat_subprograma s on n.subprograma = s.idsubprograma"; 
$query_empleado .= " left join cat_proyecto pr on n.proyecto = pr.idproyecto"; 
...
$query_empleado .= " where n.idnominaemp='$colname_empleado' and n.activo = '0'";
